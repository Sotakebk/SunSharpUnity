...
{
    internal class SunVoxLibCForEditorWithTrackedState : ISunVoxLibC
    {
        ...
        IntPtr ISunVoxLibC.sv_save_to_memory(int slot, IntPtr size) => DllImport.sv_save_to_memory(slot, size);

        int ISunVoxLibC.sv_get_base_version(int slot) => DllImport.sv_get_base_version(slot);

        int ISunVoxLibC.sv_sampler_par(int slot, int mod_num, int sample_slot, int par, int par_val, int set) => DllImport.sv_sampler_par(slot, mod_num, sample_slot, par, par_val, set);
    }
}
